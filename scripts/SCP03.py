...
def generate_test_vectors():
    """Generate SCP03 test vectors using this reference implementation"""
    from binascii import hexlify
    
    print("# SCP03 Test Vectors - Generated from asterix reference implementation")
    print()
    
    # Test Vector 1: Sequential Pattern Keys
    print("// Test Vector 1: Sequential Pattern Keys")
    
    base_enc = unhexlify("000102030405060708090A0B0C0D0E0F")
    base_mac = unhexlify("101112131415161718191A1B1C1D1E1F")
    base_dek = unhexlify("202122232425262728292A2B2C2D2E2F")
    
    host_challenge = unhexlify("0001020304050607")
    card_challenge = unhexlify("08090A0B0C0D0E0F")
    
    # Create SCP03 instance
    scp_param = {
        'i': 0x70,
        'keyVer': 0x01,
        'base_S_ENC': base_enc,
    ...
    # Initialize update to derive session keys
    scp.initUpdate(host_challenge, card_challenge=card_challenge)
    
    print(f"StaticKeyEnc = Convert.FromHexString(\"{hexlify(base_enc).decode().upper()}\");")
    print(f"StaticKeyMac = Convert.FromHexString(\"{hexlify(base_mac).decode().upper()}\");")
    print(f"StaticKeyDek = Convert.FromHexString(\"{hexlify(base_dek).decode().upper()}\");")
    print()
    ...
    print()
    print(f"ExpectedSEnc = Convert.FromHexString(\"{hexlify(scp.ses_S_ENC).decode().upper()}\");")
    print(f"ExpectedSMac = Convert.FromHexString(\"{hexlify(scp.ses_S_MAC).decode().upper()}\");")  
    print(f"ExpectedSRMac = Convert.FromHexString(\"{hexlify(scp.ses_S_RMAC).decode().upper()}\");")
    print()
    print(f"ExpectedCardCryptogram = Convert.FromHexString(\"{hexlify(scp.cardCryptogram).decode().upper()}\");")
    print(f"ExpectedHostCryptogram = Convert.FromHexString(\"{hexlify(scp.hostCryptogram).decode().upper()}\");")
    print()
    
    # Test Vector 2: All-Zeros Baseline
    print("// Test Vector 2: All-Zeros Baseline")
    
    base_enc_zero = unhexlify("00000000000000000000000000000000")
    base_mac_zero = unhexlify("00000000000000000000000000000000")
    base_dek_zero = unhexlify("00000000000000000000000000000000")
    
    host_challenge_zero = unhexlify("0000000000000000")
    card_challenge_zero = unhexlify("0000000000000000")
    
    scp_param_zero = {
        'i': 0x70,
        'keyVer': 0x01,
        'base_S_ENC': base_enc_zero,
        'base_S_MAC': base_mac_zero,
        'base_DEK': base_dek_zero,
        'SD_AID': unhexlify('A000000151000000')
    }
    
    scp_zero = SCP03(**scp_param_zero)
    scp_zero.initUpdate(host_challenge_zero, card_challenge=card_challenge_zero)
    
    print(f"StaticKeyEnc = Convert.FromHexString(\"{hexlify(base_enc_zero).decode().upper()}\");")
    print(f"StaticKeyMac = Convert.FromHexString(\"{hexlify(base_mac_zero).decode().upper()}\");")
    print(f"StaticKeyDek = Convert.FromHexString(\"{hexlify(base_dek_zero).decode().upper()}\");")
    print()
    print(f"HostChallenge = Convert.FromHexString(\"{hexlify(host_challenge_zero).decode().upper()}\");")
    print(f"CardChallenge = Convert.FromHexString(\"{hexlify(card_challenge_zero).decode().upper()}\");")
    print()
    print(f"ExpectedSEnc = Convert.FromHexString(\"{hexlify(scp_zero.ses_S_ENC).decode().upper()}\");")
    print(f"ExpectedSMac = Convert.FromHexString(\"{hexlify(scp_zero.ses_S_MAC).decode().upper()}\");")
    print(f"ExpectedSRMac = Convert.FromHexString(\"{hexlify(scp_zero.ses_S_RMAC).decode().upper()}\");")
    print()
    print(f"ExpectedCardCryptogram = Convert.FromHexString(\"{hexlify(scp_zero.cardCryptogram).decode().upper()}\");")
    print(f"ExpectedHostCryptogram = Convert.FromHexString(\"{hexlify(scp_zero.hostCryptogram).decode().upper()}\");")
    print()
