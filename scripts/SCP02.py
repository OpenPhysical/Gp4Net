...
def generate_test_vectors():
    """Generate SCP02 test vectors using this reference implementation"""
    from binascii import hexlify
    
    print("# SCP02 Test Vectors - Generated from asterix reference implementation")
    print()
    
    # Test Vector 1: i=0x15 (Static MAC)
    print("// Test Vector 1: SCP02 i=15 Static MAC with GP Test Keys")
    ...
    card_challenge = unhexlify("AABBCCDDEE112233")
    sequence_counter = unhexlify("0001")
    
    # Create SCP02 instance
    scp = SCP02(0x15, 0x01, base_enc, base_mac, base_dek, unhexlify('A000000151000000'))
    
    # Derive session keys
    scp.deriveSessionKeys(sequence_counter)
    
    print(f"StaticEncKey = Convert.FromHexString(\"{hexlify(base_enc).decode().upper()}\");")
    print(f"StaticMacKey = Convert.FromHexString(\"{hexlify(base_mac).decode().upper()}\");")
    print(f"StaticDekKey = Convert.FromHexString(\"{hexlify(base_dek).decode().upper()}\");")
    print()
    ...
    print(f"SequenceCounter = Convert.FromHexString(\"{hexlify(sequence_counter).decode().upper()}\");")
    print("ImplementationOption = 0x15;")
    print()
    print(f"ExpectedSEncKey = Convert.FromHexString(\"{hexlify(scp.ses_S_ENC).decode().upper()}\");")
    print(f"ExpectedSMacKey = Convert.FromHexString(\"{hexlify(scp.ses_S_MAC).decode().upper()}\");")
    print(f"ExpectedSDekKey = Convert.FromHexString(\"{hexlify(scp.ses_DEK).decode().upper()}\");")
    print()
    
    # Generate cryptogram data
    card_cryptogram_data = scp.cardCryptogramData(host_challenge, card_challenge, sequence_counter)
    host_cryptogram_data = scp.hostCryptogramData(host_challenge, card_challenge, sequence_counter)
    
    print(f"CardCryptogramData = Convert.FromHexString(\"{hexlify(card_cryptogram_data).decode().upper()}\");")
    print(f"HostCryptogramData = Convert.FromHexString(\"{hexlify(host_cryptogram_data).decode().upper()}\");")
    print()
    
    # Generate cryptograms
    card_cryptogram = scp.cardCryptogram(host_challenge, card_challenge, sequence_counter)
    host_cryptogram = scp.hostCryptogram(host_challenge, card_challenge, sequence_counter)
