...
namespace Gp4Net.Domain.Protocol
{
    ...
    public abstract class SecureChannelProtocolBase : ISecureChannelProtocol
    {
        ...
        /// <inheritdoc />
        public abstract SecureChannelSession CreateSecureChannelSession(
            SecureChannelContext context,
            SecurityLevel securityLevel);
        ...
        {
            return BuildCardCryptogramData(response, hostChallenge)
                .Map(cryptogramData =>
                {
                    var cryptogramContext = new Cryptography.Implementation.CryptogramContext(
                        protocolVersion: ProtocolVersion,
                        key: sessionKeys.SEnc, // Use S-ENC key for cryptograms per GP specification
                        data: cryptogramData,
                        type: Cryptography.CryptogramType.CardCryptogram
                    );
        ...
        {
            return BuildHostCryptogramData(context.InitializeUpdateResponse, context.HostChallenge)
                .Map(cryptogramData =>
                {
                    var cryptogramContext = new Cryptography.Implementation.CryptogramContext(
                        protocolVersion: ProtocolVersion,
                        key: context.SessionKeys.SEnc, // Use S-ENC key for cryptograms per GP specification
                        data: cryptogramData,
                        type: Cryptography.CryptogramType.HostCryptogram
                    );
