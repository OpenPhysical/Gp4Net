...
)
{
    ...
    public Maybe<ApplicationTag> GpVersionTag =>
        ApplicationTags
            .Where(t => t.TagNumber == 0x60)
            .ToList() is var tags && tags.Count > 0
    ...
    public IReadOnlyList<string> GetAllOids()
    {
        ...
        var nestedOids = ApplicationTags
            .SelectMany(tag => tag.NestedOid
                .Map(oid => new[] { oid })
                .GetValueOrDefault(Enumerable.Empty<string>().ToArray()));
    ...
    /// </summary>
    public static CardRecognitionData Empty => new(
        Maybe<string>.None,
        ImmutableList<ApplicationTag>.Empty
    );
}