...
namespace Gp4Net.Domain.CardInfo
{
    ...
    public static class ScpCapabilitiesParser
    {
        ...
        public static string Parse(byte[] data)
        {
            ...
            try
            {
                // Use SimpleTlvParser to properly parse TLV structure
                foreach (var element in SimpleTlvParser.Enumerate(data))
                {
                    switch (element.Tag)
                    {
                        case 0x81: // Secure messaging support
                            foreach (var protocolByte in element.Content)
                            {
                        ...
                        case 0x82: // Secure channel protocol data
                            ...
                            foreach (var valueByte in element.Content)
                            {
                        ...
                        case 0x83: // Additional security capabilities
                            ...
                            {
                                var capabilities = element.Content[0];
