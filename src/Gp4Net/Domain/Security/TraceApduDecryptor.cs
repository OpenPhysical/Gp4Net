...
/// </summary>
[PublicAPI]
public sealed class TraceApduDecryptor
{
    private readonly ILogger<TraceApduDecryptor> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="TraceApduDecryptor"/> class.
    /// </summary>
    /// <param name="logger">The logger instance. If null, uses NullLogger.</param>
    public TraceApduDecryptor(ILogger<TraceApduDecryptor>? logger = null)
    {
        _logger = logger ?? NullLogger<TraceApduDecryptor>.Instance;
    }
    ...
    )
    {
        ...
        // Calculate expected MAC using CryptoOperations
        ...
        var expectedMacResult =
            sessionState.ProtocolVersion == ScpVersion.Scp03
                ? CryptoOperations.ScpOperations.Scp03.CalculateCommandMac(
                    macInput,
                    sessionState.SessionKeys.SMac,
                    commandMacChainingBytes
                )
                : CryptoOperations.ScpOperations.Scp02.CalculateCommandMac(
                    macInput,
                    sessionState.SessionKeys.SMac,
                    commandMacChainingBytes
                );
    ...
    )
    {
        ...
        // Use ScpOperations to remove security (verify MAC and decrypt)
        ...
        var decryptionResult =
            direction == ApduDirection.Command
                ? apduBytes
                    .ParseCommandApdu()
                    .Bind(cmd =>
                        Gp4Net.Services.ScpOperations.Security.RemoveCommandSecurity(
                            cmd,
                            sessionState
                        )
                    )
                ...
                : apduBytes
                    .ParseResponseApdu()
                    .Bind(resp =>
                        Gp4Net.Services.ScpOperations.Security.RemoveResponseSecurity(
                            resp,
                            sessionState
                        )
                    )
    ...
    )
    {
        ...
        // Calculate expected MAC based on protocol version using CryptoOperations
        var macChainingBytes = sessionState.MacChainingValue;

        var macResult = sessionState.ProtocolVersion switch
        {
            ScpVersion.Scp02
                => CryptoOperations.ScpOperations.Scp02.CalculateResponseMac(
                    responseData.Concat(statusWord).ToArray(),
                    sessionState.SessionKeys.SrMac,
                    macChainingBytes
                ),
            ScpVersion.Scp03
                => CryptoOperations.ScpOperations.Scp03.CalculateResponseMac(
                    responseData.Concat(statusWord).ToArray(),
                    sessionState.SessionKeys.SrMac,
                    macChainingBytes
                ),
        ...
        return macResult.Bind(expectedMac =>
        {
            ...
            bool macValid = CryptoOperations.Utils.CompareBytes(expectedToCompare, receivedMac);
