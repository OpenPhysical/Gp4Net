    ...
    public static class Validation
    {
                    ...
                    Some: _ =>
                        Maybe<byte[]>
                            ...
                            .Match(
                                ...
                                None: () => UnitResult.Failure(Errors.NullArgument("Data"))
                            ),
                    None: () => UnitResult.Failure(Errors.NullArgument("Key"))
                );
        }
                                ...
                                Some: _ =>
                                    Maybe<byte[]>
                                        ...
                                        .Match(
                                            Some: _ => UnitResult.Success<SmartCardError>(),
                                            None: () =>
                                                UnitResult.Failure(Errors.NullArgument("Data"))
                                        ),
                                None: () => UnitResult.Failure(Errors.NullArgument("IV"))
                            ),
                    None: () => UnitResult.Failure(Errors.NullArgument("Key"))
                );
        }
        ...
        public static UnitResult<SmartCardError> ValidateInput(byte[] input, string parameterName)
        {
            return Maybe<byte[]>.From(input).HasValue
                ? UnitResult.Success<SmartCardError>()
                : UnitResult.Failure(Errors.EmptyArgument(parameterName));
        }
        ...
        )
        {
            return input.Length == expectedLength
                ? UnitResult.Success<SmartCardError>()
                : UnitResult.Failure(
                    Errors.InvalidLength(parameterName, expectedLength, input.Length)
                );
        }
