...
/// ONLY orchestrates by calling CryptoOperations - contains NO crypto logic itself.
/// </summary>
public static partial class ScpOperations
{
    /// <summary>
    /// Secure command and response processing operations.
    /// Orchestrates security application by delegating to CryptoOperations.
    /// </summary>
    [PublicAPI]
    public static class Security
    {
        ...
        private static readonly ILogger Logger = NullLoggerFactory.Instance.CreateLogger(
            "Gp4Net.Services.ScpOperations.Security"
        );
        ...
        /// Orchestrates by calling the appropriate CryptoOperations operations.
        /// </summary>
        ...
        > ApplyCommandSecurity(CommandAPDU command, SecureChannelState state)
        {
            ...
            {
                CryptoOperations.ScpVersion.Scp02 => ApplyScp02CommandSecurity(command, state),
                CryptoOperations.ScpVersion.Scp03 => ApplyScp03CommandSecurity(command, state),
                _
        ...
        > RemoveCommandSecurity(CommandAPDU securedCommand, SecureChannelState state)
        {
            return state.Protocol switch
            {
                CryptoOperations.ScpVersion.Scp02
                    => RemoveScp02CommandSecurity(securedCommand, state),
                CryptoOperations.ScpVersion.Scp03
                    => RemoveScp03CommandSecurity(securedCommand, state),
                _
                    => Result.Failure<(CommandAPDU, SecureChannelState), SmartCardError>(
                        SmartCardError.InvalidArgument($"Unsupported protocol: {state.Protocol}")
                    ),
        ...
        > RemoveResponseSecurity(ResponseAPDU securedResponse, SecureChannelState state)
        {
            ...
            if (
                state.Protocol == CryptoOperations.ScpVersion.Scp03
        ...
        > ApplyScp03ResponseSecurity(ResponseAPDU plaintextResponse, SecureChannelState state)
        {
            ...
            Result<byte[], SmartCardError> protectedResponse = state.HasResponseEncryption
                ? CryptoOperations.ScpOperations.Scp03.ApplyResponseEncryption(
        ...
        /// Complete secure command pipeline using CryptoOperations operations.
        /// </summary>
        /// <param name="transmit">The card service for communication.</param>
        ...
        )
        {
            return await ApplyCommandSecurity(command, state)
                .Bind(async secured =>
                {
                    ...
                    return await transmit(securedCommand.BinaryCommand, cancellationToken)
                        .Bind(response =>
        ...
        > ApplyScp02CommandSecurity(CommandAPDU command, SecureChannelState state)
        {
            ...
            var icvToUse =
                (command.Ins == 0x82 || !implementation.HasIcvEncryption())
                    ...
                    : CryptoOperations.Mac.EncryptScp02Icv(
                        state.MacChainingValue,
            ...
            return command
                ...
                .Bind(macInput =>
                    icvToUse.Bind(icv =>
                        CryptoOperations.Mac.CalculateScp02CommandMac(
                            state.SessionKeys.SMac,
                            macInput.Bytes,
                            icv
                        )
        ...
        > RemoveScp03ResponseSecurity(ResponseAPDU securedResponse, SecureChannelState state)
        {
            ...
            var macCalculationResult = CryptoOperations.ScpOperations.Scp03.CalculateResponseMac(
                responseWithoutMac,
                state.SessionKeys.SrMac,
                state.MacChainingValue
            );
            ...
            var plaintextResult =
                ...
                && CryptoOperations.ScpOperations.Common.HasResponseData(responseWithoutMac)
                    ? CryptoOperations.ScpOperations.Scp03.RemoveResponseEncryption(
                        responseWithoutMac,
                        state.SessionKeys.SEnc,
                        state.EncryptionCounter
                    )
    ...
    public static class Validation
    {
        ...
        public static UnitResult<SmartCardError> ValidateHostChallenge(byte[] hostChallenge)
        {
            if (hostChallenge is null)
                return UnitResult.Failure(Errors.NullArgument("Host challenge"));

            return hostChallenge.Length == 8
                ? UnitResult.Success<SmartCardError>()
                : UnitResult.Failure(
                    Errors.InvalidLength("Host challenge", 8, hostChallenge.Length)
                );
        }
        ...
        )
        {
            if (cardChallenge is null)
                return UnitResult.Failure(Errors.NullArgument("Card challenge"));
        ...
        )
        {
            if (sequenceCounter is null)
                return UnitResult.Failure(Errors.NullArgument("Sequence counter"));
        ...
        public static UnitResult<SmartCardError> ValidateProtocolVersion(
            Maybe<CryptoOperations.ScpVersion> responseScpId,
            CryptoOperations.ScpVersion expectedProtocol
        ) =>
            responseScpId.Match(
                actualProtocol =>
        ...
        {
            if (keySet is null)
                return UnitResult.Failure(Errors.NullArgument("Key set"));
