...
/// <summary>
/// Instance-based implementation of IGlobalPlatformService.
/// This class will be deprecated in favor of the static GlobalPlatformService.
/// </summary>
public class GlobalPlatformServiceInstance : IGlobalPlatformService
{
    private readonly ISmartCardService _cardService;
    private readonly IKeysetResolver _keysetResolver;
    private readonly ILogger<GlobalPlatformServiceInstance> _logger;

    /// <inheritdoc/>
    public ISmartCardService CardService => _cardService;

    /// <summary>
    /// Initializes a new instance of the GlobalPlatformServiceInstance class.
    /// </summary>
    public GlobalPlatformServiceInstance(
        ISmartCardService cardService,
        IKeysetResolver keysetResolver,
        ILogger<GlobalPlatformServiceInstance> logger
    )
    {
    ...
    )
    {
        ...
        // Use the GlobalPlatformService Discovery
        Result<SelectResponse, SmartCardError> result = await GlobalPlatformService.Discovery.DetectAndSelectIsdAsync(
    ...
    {
        _logger.LogInformation(
            "Establishing secure channel with keyset: {KeysetName}, security level: {SecurityLevel}",
            keysetName,
            securityLevel
        );
        ...
        Result<InitializeUpdateResponse, SmartCardError> initUpdateResult =
            await SendInitializeUpdateForDiversificationAsync(keyVersion, cancellationToken);
        ...
        Result<IKeySet, SmartCardError> keySetResult = initUpdateResult.Bind(initResponse =>
            _keysetResolver.ResolveKeyset(
                keysetName,
                new Dictionary<string, string>(), // Empty parameters for named keysets
                Maybe<byte[]>.None,
                Maybe<byte[]>.None,
                Maybe<byte[]>.None, // No explicit keys
                keyVersion,
        ...
        return await keySetResult.Match(
            keySet =>
                keySet switch
                {
                    KeySet concreteKeySet => EstablishSecureChannelAsync(
                        concreteKeySet,
                        securityLevel,
                        cancellationToken
                    ),
                    _ => Task.FromResult(
                        Result.Failure<SecureChannelState, SmartCardError>(
                            SmartCardError.CryptographicError(
                                "Resolved keyset is not a concrete KeySet instance"
                            )
                        )
                    ),
                },
    ...
    {
        _logger.LogInformation(
            "Establishing secure channel with explicit keys, security level: {SecurityLevel}",
            securityLevel
        );
        ...
        return await encKeyResult.Bind(encBytes =>
            macKeyResult.Bind(macBytes =>
                dekKeyResult.Bind(dekBytes =>
                    EstablishWithExplicitKeys(
                        encBytes,
                        macBytes,
                        dekBytes,
                        keyVersion,
    ...
    )
    {
        ...
        Result<IKeySet, SmartCardError> keySetResult = initUpdateResult.Bind(initResponse =>
            _keysetResolver.ResolveKeyset(
                ...
                Maybe<byte[]>.From(dekKey), // Explicit keys
                keyVersion,
                Maybe<InitializeUpdateResponse>.From(initResponse)
            )
        );
    ...
    > SendInitializeUpdateForDiversificationAsync(
        byte keyVersion,
        CancellationToken cancellationToken
    )
    {
        Result<byte[], SmartCardError> hostChallengeResult =
            CryptoService.Utils.GenerateHostChallenge();
        if (hostChallengeResult.IsFailure)
        {
            return Result.Failure<InitializeUpdateResponse, SmartCardError>(
                hostChallengeResult.Error
            );
        }
        ...
        Result<InitializeUpdateCommand, SmartCardError> commandResult =
            InitializeUpdateCommand.Create(keyVersion, 0x00, hostChallenge);
        ...
        // Execute the command
        Result<CommandResponse, SmartCardError> response = await _cardService.ExecuteCommandAsync(
            commandResult.Value,
            cancellationToken
        );
    ...
    )
    {
        ...
        // Create the GET STATUS command and execute it
        var cmdResult = GlobalPlatformService.Commands.CreateGetStatusCommand(
            (StatusSubset)(byte)subset,
            null
        );
        ...
        var response = await _cardService.ExecuteCommandAsync(cmdResult.Value, cancellationToken);
        
        return response.IsSuccess
            ? GlobalPlatformService.Responses.ParseGetStatusResponse(response.Value)
            : Result.Failure<ImmutableList<ApplicationInfo>, SmartCardError>(response.Error);
    }

    /// <inheritdoc/>
    public async Task<Result<Results.InstallationResult, SmartCardError>> InstallCapFileAsync(
        byte[] capFileData,
    ...
    )
    {
        ...
        Maybe<SecureChannelState> session = _cardService.Context.Get<SecureChannelState>(
            ContextKeys.SecureChannelSession
        );
        if (!session.HasValue)
        {
            return Result.Failure<Results.InstallationResult, SmartCardError>(
        ...
        Result<IList<IApduCommand>, SmartCardError> createCommandsResult =
            CapFileLoadingWorkflow.CreateLoadingCommands(capFileData);

        if (createCommandsResult.IsFailure)
        {
            return Result.Failure<Results.InstallationResult, SmartCardError>(createCommandsResult.Error);
        }

        IList<IApduCommand> commands = createCommandsResult.Value;
        _logger.LogInformation(
            "Created {Count} installation commands from CAP file",
            commands.Count
        );

        // Execute all commands through the card service using functional composition
        Task<Result<(IApduCommand command, CommandResponse response), SmartCardError>>[] tasks =
            [.. commands
                .Select(async command =>
                    (await _cardService.ExecuteCommandAsync(command, cancellationToken)).Map(
                        response => (command, response)
                    )
                )];

        Result<(IApduCommand command, CommandResponse response), SmartCardError>[] results =
            await Task.WhenAll(tasks);

        // Check for any failures - fail fast on first error
        foreach (
            Result<
                (IApduCommand command, CommandResponse response),
                SmartCardError
            > result in results
        )
        {
            if (result.IsFailure)
            {
                return Result.Failure<Results.InstallationResult, SmartCardError>(result.Error);
            }
        }

        // All successful - extract values
        (IApduCommand command, CommandResponse response)[] successfulResults = [.. results.Select(r => r.Value)];

        _logger.LogInformation(
            "Successfully installed CAP file with {Count} commands",
            successfulResults.Length
        );

        return Result.Success<Results.InstallationResult, SmartCardError>(
            new Results.InstallationResult(
    ...
    private static byte[] ExtractPackageAidFromResults(
        (IApduCommand command, CommandResponse response)[] results
    )
    {
    ...
    private static ImmutableList<byte[]> ExtractInstalledAppletsFromResults(
        (IApduCommand command, CommandResponse response)[] results
    )
    {
        return [.. results
    ...
    )
    {
        ...
        return await GlobalPlatformService.CardManagement.DeleteApplicationAsync(
            aid,
            deleteRelated,
            async (cmd, ct) => await _cardService.ExecuteCommandAsync(cmd, ct),
            cancellationToken
    ...
    )
    {
        ...
        Result<PutKeyCommand, SmartCardError> commandResult = PutKeyCommand
            .Create(keyVersion: keyVersion, keyDataBlocks: new List<KeyDataBlock>()) // Empty for now
            .MapError(e =>
                SmartCardError.InvalidData($"Failed to create PUT KEY command: {e.Message}")
            );
    ...
    {
        _logger.LogInformation(
            "Setting lifecycle state for AID: {AID} to {State}",
        ...
        // Use GlobalPlatformService CardManagement
        return await GlobalPlatformService.CardManagement.SetLifecycleStateAsync(
            aid,
            (byte)state,
            async (cmd, ct) => await _cardService.ExecuteCommandAsync(cmd, ct),
            cancellationToken
    ...
    )
    {
        ...
        string readerName = _cardService
            .Context.Get<string>("ReaderName")
        ...
        return Result.Success<CardInformation, SmartCardError>(
            new CardInformation(
                ...
                HistoricalBytes: Maybe<byte[]>.None
            )
        ); // Could be enhanced
    }
    ...
    {
        _logger.LogDebug(
            "Establishing secure channel directly with protocol version {Protocol}",
            keySet.KeyVersion
        );

        // Detect protocol version from keyset
        CryptoService.ScpVersion protocolVersion = keySet switch
        {
            { KeyVersion: 0x02 } => CryptoService.ScpVersion.Scp02,
            { KeyVersion: 0x03 } => CryptoService.ScpVersion.Scp03,
            _ => CryptoService.ScpVersion.Scp02, // Default fallback
        };

        // Generate host challenge using UnifiedCryptoService
        Result<byte[], SmartCardError> hostChallengeResult = CryptoService.Utils.GenerateHostChallenge();
        if (hostChallengeResult.IsFailure)
        {
            return Result.Failure<SecureChannelState, SmartCardError>(hostChallengeResult.Error);
        }
        byte[] hostChallenge = hostChallengeResult.Value;

        return protocolVersion switch
        {
            CryptoService.ScpVersion.Scp02 => await EstablishScp02ChannelDirectly(
                keySet,
                securityLevel,
                hostChallenge,
                cancellationToken
            ),
            CryptoService.ScpVersion.Scp03 => await EstablishScp03ChannelDirectly(
                keySet,
                securityLevel,
                hostChallenge,
                cancellationToken
    ...
    )
    {
        ...
        return await InitializeUpdateCommand
            .Create(keySet.KeyVersion, keySet.KeyId, hostChallenge)
            .Bind(async initUpdateCmd =>
            {
                // Send INITIALIZE UPDATE
                var initResponse = await _cardService.ExecuteCommandAsync(
                    initUpdateCmd,
                    cancellationToken
                );
                return await initResponse
                    ...
                    .Bind(response =>
                        InitializeUpdateResponse
                            ...
                            .Bind(parsedResponse =>
                                Scp02Protocol
                                    .ProcessInitializeUpdateResponse(
                                        parsedResponse,
                                        hostChallenge,
                                        keySet
                                    )
                                    .Bind(context =>
                                        CompleteScp02Authentication(
                                            context,
                                            securityLevel,
                                            cancellationToken
                                        )
    ...
    {
        return await Scp02Protocol
            .CreateExternalAuthenticateCommand(context, securityLevel)
            .Bind(async extAuthCmd =>
            {
                var extAuthResponse = await _cardService.ExecuteCommandAsync(
                    extAuthCmd,
                    cancellationToken
                );
                return extAuthResponse.Bind(response =>
                    response.IsSuccess
    ...
    {
        return await CryptoService.Cryptogram
            .BuildScp03HostCryptogramData(context.InitializeUpdateResponse, hostChallenge)
            .Bind(hostCryptogramData =>
                CryptoService.Cryptogram
                    .CalculateScp03Cryptogram(context.SessionKeys.SEnc, hostCryptogramData)
                    .Bind(cryptogram =>
                    {
                        byte[] truncatedCryptogram = [.. cryptogram.Take(8)];
                        return Scp03Protocol.CreateExternalAuthenticateCommand(
                            securityLevel,
                            truncatedCryptogram,
