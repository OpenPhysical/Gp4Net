...
public class CardContentRetriever
{
    ...
    )
    {
        ...
        Result<SelectResponse, SmartCardError> selectResult =
            await GlobalPlatformService.Discovery.DetectAndSelectIsdAsync(
        ...
        return await GlobalPlatformService.Applications.RetrieveCompleteCardContentAsync(
            async (cmd, ct) => await _cardService.ExecuteCommandAsync(cmd, ct),
            cancellationToken
        );
    }
                ...
                return await ConvertToKeySet(ks)
                    .Bind(async keySetForGp =>
                    {
                        Result<SecureChannelState, SmartCardError> secureChannelResult =
                            await _gpService.EstablishSecureChannelAsync(keySetForGp);
