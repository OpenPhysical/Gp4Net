...
/// and ALL crypto operations to CryptoOperations.cs.
///
...
{
    /// <summary>
    /// The side-effect boundary required by secure-channel establishment.
    /// </summary>
    public delegate Task<Result<CommandResponse, SmartCardError>> Transmit(
        byte[] command,
    ...
    public static class Types
    {
        ...
        public sealed record ScpOption(
            CryptoOperations.ScpVersion Protocol, // 0x02 or 0x03
            ...
            private static UnitResult<SmartCardError> ValidateImplementation(
                ...
                protocol switch
                {
                    CryptoOperations.ScpVersion.Scp02
                        => CryptoOperations.ScpOperations.Common.IsValidScp02Implementation(
                            implementation
                        )
                    ...
                    CryptoOperations.ScpVersion.Scp03
                        => CryptoOperations.ScpOperations.Common.IsValidScp03Implementation(
                            implementation
                        )
                            ? UnitResult.Success<SmartCardError>()
                            : UnitResult.Failure(
    ...
    /// Orchestrates the complete SCP handshake process by calling CryptoOperations.
    /// </summary>
    [PublicAPI]
    public static class Establishment
    {
        ...
        /// Orchestrates by calling CryptoOperations operations in sequence.
        /// </summary>
        ...
        {
            var cardCapabilities = await QueryCardCapabilities(transmit, cancellationToken);
        ...
        )
        {
            return await Task.FromResult(
                diversification.Match(
                    spec =>
                        KeyDiversification.DiversifyScp03KeySet(
                            keySet,
                            spec,
        ...
        )
        {
            ...
            {
                CryptoOperations.ScpVersion.Scp02 when keySet is Scp02KeySet scp02KeySet
                    => await EstablishScp02Async(
                        transmit,
                        scp02KeySet,
                ...
                CryptoOperations.ScpVersion.Scp03 when keySet is Scp03KeySet scp03KeySet
                    => await EstablishScp03Async(
                        transmit,
                        scp03KeySet,
                        securityLevel,
        ...
        private static async Task<Result<CommandResponse, SmartCardError>> TryGetData(
            ...
            await GetDataCommand
                ...
                .Bind(async bytes => await transmit(bytes, cancellationToken));
        ...
        private static async Task<Result<byte[], SmartCardError>> TryGetDataOrEmpty(
            ...
            (await TryGetData(tagValue, transmit, cancellationToken))
                .Bind(ExtractSuccessfulData)
                .Match(
        ...
        private static UnitResult<SmartCardError> VerifyScp02CardCryptogram(
            ...
            CryptoOperations
                ...
                .Bind(data =>
                    CryptoOperations.ScpOperations.Scp02.CalculateCryptogram(sessionKeys.SEnc, data)
                )
                .Bind(calculated =>
                    CryptoOperations.Utils.CompareBytes(calculated, response.CardCryptogram)
                        ? UnitResult.Success<SmartCardError>()
                        : UnitResult.Failure(
        ...
        > CreateScp02ExternalAuthenticate(
            ...
            CryptoOperations
                ...
                .Bind(data =>
                    CryptoOperations.ScpOperations.Scp02.CalculateCryptogram(
        ...
        {
            if (
                context.Protocol == CryptoOperations.ScpVersion.Scp02
            ...
            if (
                context.Protocol == CryptoOperations.ScpVersion.Scp03
            ...
            return CryptoOperations.ScpOperations.Common.GetZeroChainingValue(context.Protocol);
        }
