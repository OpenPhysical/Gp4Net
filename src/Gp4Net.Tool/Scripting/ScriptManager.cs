...
public class ScriptManager : IScriptManager
{
    ...
    )
    {
        ...
        var scriptName = "kdf";
        var functionName = scriptFunction;
    ...
    private static void RegisterCryptoModule(Script script)
    {
        var cryptoModule = new CryptoScriptModule();
        script.Globals["crypto"] = UserData.Create(cryptoModule);
    }

    private static void RegisterUtilityModule(Script script)
    {
        var utilityModule = new UtilityScriptModule();

        // Register as global functions for convenience
        script.Globals["hex"] = (Func<string, byte[]>)UtilityScriptModule.Hex;
        script.Globals["bytes"] = (Func<object, byte[]>)utilityModule.Bytes;
        script.Globals["concat"] = (Func<byte[][], byte[]>)UtilityScriptModule.Concat;
        script.Globals["sub"] = (Func<byte[], int, int, byte[]>)UtilityScriptModule.Sub;
        script.Globals["xor"] = (Func<byte[], byte[], byte[]>)UtilityScriptModule.Xor;
        script.Globals["pad80"] = (Func<byte[], int, byte[]>)UtilityScriptModule.Pad80;
        script.Globals["hex_string"] = (Func<byte[], string>)UtilityScriptModule.HexString;
        script.Globals["random_bytes"] = (Func<int, byte[]>)UtilityScriptModule.RandomBytes;
    }
