...
namespace Gp4Net.Tool.Scripting
{
    ...
    public class CryptoScriptModule
    {
        ...
        public byte[] DesEcb(byte[] key, byte[] data)
        {
            if (key.Length != 8)
            {
        ...
        /// </summary>
        [MoonSharpVisible(true)]
        public byte[] DesCbc(byte[] key, byte[] iv, byte[] data)
        {
            if (key.Length != 8)
            {
                throw new ArgumentException("DES key must be 8 bytes");
        ...
        /// </summary>
        [MoonSharpVisible(true)]
        public byte[] Des3Ecb(byte[] key, byte[] data)
        {
            ...
            if (key.Length == 16)
            {
                var expandedKey = new byte[24];
                Array.Copy(key, 0, expandedKey, 0, 16);
                Array.Copy(key, 0, expandedKey, 16, 8);
                key = expandedKey;
            }
        ...
        /// </summary>
        [MoonSharpVisible(true)]
        public byte[] Des3Cbc(byte[] key, byte[] iv, byte[] data)
        {
            if (key.Length != 16 && key.Length != 24)
            {
                throw new ArgumentException("3DES key must be 16 or 24 bytes");
        ...
        /// </summary>
        [MoonSharpVisible(true)]
        public byte[] AesEcb(byte[] key, byte[] data)
        {
        ...
        /// </summary>
        [MoonSharpVisible(true)]
        public byte[] AesCbc(byte[] key, byte[] iv, byte[] data)
        {
            if (key.Length != 16 && key.Length != 24 && key.Length != 32)
            {
                throw new ArgumentException("AES key must be 16, 24, or 32 bytes");
        ...
        /// </summary>
        [MoonSharpVisible(true)]
        public byte[] CmacDes(byte[] key, byte[] data)
        {
        ...
        /// </summary>
        [MoonSharpVisible(true)]
        public byte[] CmacAes(byte[] key, byte[] data)
        {
        ...
        /// </summary>
        [MoonSharpVisible(true)]
        public byte[] Iso9797Mac(byte[] key, byte[] data)
        {
