...
namespace Gp4Net.Tool.Commands.Applet
{
    ...
    public class ValidateCommand : BaseCommand<ValidateCommand.Settings>
    {
        ...
        /// </summary>
        public ValidateCommand(ICardService cardService, IGlobalPlatformService globalPlatformService, PackageRegistry packageRegistry)
        ...
        protected override async Task<int> ExecuteCommandAsync(CommandContext context, Settings settings)
        {
            ...
            {
                AnsiConsole.MarkupLine($"[red]CAP file not found: {Markup.Escape(settings.CapFile)}[/]");
                return 1;
            }

            try
            {
                AnsiConsole.MarkupLine($"[cyan]Validating CAP file: {Markup.Escape(settings.CapFile)}[/]");
                ...
                if (validationResult.IsValid)
                {
                    ...
                    table.AddRow("Format", "ZIP/JAR");
                    table.AddRow("Package AID", $"[dim]{Convert.ToHexString(capFile.PackageAid)}[/]");
                    table.AddRow("Package Version", $"{capFile.PackageVersion.Major}.{capFile.PackageVersion.Minor}");
                    table.AddRow("CAP File Version", $"{capFile.CapFileVersion.Major}.{capFile.CapFileVersion.Minor}");
                    
                    // Interpret header flags
                    var flagsInterpreted = new List<string>();
                    if ((capFile.HeaderFlags & 0x01) != 0) flagsInterpreted.Add("INT");
                    if ((capFile.HeaderFlags & 0x02) != 0) flagsInterpreted.Add("EXPORT");
                    if ((capFile.HeaderFlags & 0x04) != 0) flagsInterpreted.Add("APPLET");
                    var flagsDisplay = flagsInterpreted.Count > 0 
                        ? $"0x{capFile.HeaderFlags:X2} ({string.Join(", ", flagsInterpreted)})"
                        : $"0x{capFile.HeaderFlags:X2}";
                    table.AddRow("Header Flags", flagsDisplay);
                    
                    table.AddRow("Total Size", $"{capFile.TotalSize} bytes");
                    table.AddRow("Components", capFile.Components.Count.ToString());
                    table.AddRow("Applets", capFile.Applets.Count.ToString());
                    ...
                    table.AddRow("Est. Load Blocks", estimatedBlocks.ToString());
                ...
                {
                    AnsiConsole.MarkupLine($"[red]✗ CAP file validation failed: {Markup.Escape(validationResult.ErrorMessage ?? "Unknown error")}[/]");
                    return 1;
                }
            ...
            {
                AnsiConsole.MarkupLine($"[red]Error validating CAP file: {Markup.Escape(ex.Message)}[/]");
                if (settings.Verbose)
                {
        ...
        private static void DisplayDetailedInformation(CapFileStructure capFile, bool verbose)
        {
            ...
            if (capFile.Applets.Count > 0)
            {
                ...
                var appletsTable = new Table()
                    .AddColumn("AID")
                    .AddColumn("Install Method Offset");
        ...
        private static void DisplaySecurityAnalysis(CapFileStructure capFile)
        {
            ...
            if ((capFile.HeaderFlags & 0x01) != 0) capabilities.Add("Requires 32-bit integer support");
            if ((capFile.HeaderFlags & 0x02) != 0) capabilities.Add("Exports APIs to other packages");
            if ((capFile.HeaderFlags & 0x04) != 0) capabilities.Add("Contains installable applets");
            
            if (capabilities.Count > 0)
                securityTable.AddRow("Capabilities", string.Join("\n", capabilities));
            else
                securityTable.AddRow("Capabilities", "[dim]None declared[/]");

            // Check for sensitive components
            var hasExport = capFile.Components.Any(c => c.Tag == CapFileStructure.ComponentTags.Export);
            var hasDebug = capFile.Components.Any(c => c.Tag == CapFileStructure.ComponentTags.Debug);
            
            var sensitiveComponents = new List<string>();
            if (hasExport) sensitiveComponents.Add("Export component present (exposes APIs)");
            if (hasDebug) sensitiveComponents.Add("[yellow]Debug component present (may contain sensitive info)[/]");
            
            if (sensitiveComponents.Count > 0)
                securityTable.AddRow("Sensitive Components", string.Join("\n", sensitiveComponents));

            // Analyze imports for crypto usage
            if (capFile.Manifest?.ImportedPackages != null && capFile.Manifest.ImportedPackages.Count > 0)
            {
                ...
                if (cryptoImports.Count > 0)
                    securityTable.AddRow("Crypto Usage", string.Join("\n", cryptoImports));
            }

            // Static field analysis summary
            var staticFieldComponent = capFile.Components.FirstOrDefault(c => c.Tag == CapFileStructure.ComponentTags.StaticField);
            if (staticFieldComponent != null && staticFieldComponent.Size > 0)
            {
                securityTable.AddRow("Static Data", $"{staticFieldComponent.Size} bytes (use --verbose to inspect)");
            }
            ...
            if (capFile.Applets.Count > 0)
            {
                ...
                securityTable.AddRow("Installable Applets", string.Join("\n", appletInfo));
            }
        ...
        private static void DisplayMemoryEstimate(byte[] capFileData)
        {
            try
            {
                ...
                var memoryTable = new Table()
                    .AddColumn("Memory Type")
                    .AddColumn("Estimated Size");

                memoryTable.AddRow("Code Memory", $"{memoryReq.CodeMemory} bytes");
                memoryTable.AddRow("Data Memory", $"{memoryReq.DataMemory} bytes");
                memoryTable.AddRow("Total Size", $"{memoryReq.TotalSize} bytes");

                AnsiConsole.Write(memoryTable);
            }
            catch (Exception ex)
            {
                AnsiConsole.MarkupLine($"[yellow]Could not estimate memory requirements: {Markup.Escape(ex.Message)}[/]");
            }
        }

        private static void DisplayManifestInformation(ManifestInfo manifest, PackageRegistry packageRegistry)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(manifest.PackageName))
                    manifestTable.AddRow("Package Name", Markup.Escape(manifest.PackageName));
                
                if (!string.IsNullOrEmpty(manifest.CapFileVersion))
                    manifestTable.AddRow("CAP File Version", Markup.Escape(manifest.CapFileVersion));
                    
                if (!string.IsNullOrEmpty(manifest.ConverterVersion))
                    manifestTable.AddRow("Converter Version", Markup.Escape(manifest.ConverterVersion));
                    
                if (!string.IsNullOrEmpty(manifest.ConverterProvider))
                    manifestTable.AddRow("Converter Provider", Markup.Escape(manifest.ConverterProvider));
                    
                if (!string.IsNullOrEmpty(manifest.CreationTime))
                    manifestTable.AddRow("Creation Time", Markup.Escape(manifest.CreationTime));
                    
                if (manifest.IntegerSupportRequired.HasValue)
                    manifestTable.AddRow("Integer Support Required", manifest.IntegerSupportRequired.Value ? "Yes" : "No");
                ...
                if (manifest.ImportedPackages.Count > 0)
                {
                    ...
                    foreach (var import in manifest.ImportedPackages)
                    {
                        ...
                        {
                            resolvedName = $"[green]{packageInfo.DisplayName}[/]";
                            sdkVersion = $"[yellow]{packageInfo.SdkVersion}[/]";
                        }
            ...
            {
                AnsiConsole.MarkupLine($"[yellow]Could not display manifest information: {Markup.Escape(ex.Message)}[/]");
            }
        }
        ...
        private static void DisplayClassFileInfo(byte[] capFileData)
        {
            try
            {
                ...
                {
                    AnsiConsole.MarkupLine($"[yellow]Found {classFiles.Count} Java class files embedded in CAP[/]");
                    AnsiConsole.WriteLine();
                    ...
                    foreach (var classFile in classFiles)
                    {
                        ...
                        if (lastSlash > 0)
                        {
                            ...
                            if (packagePath.Contains("/classes/"))
                                packagePath = packagePath.Substring(packagePath.IndexOf("/classes/") + 9);
        ...
        private static void DisplayStaticFieldArrays(CapFileStructure capFile)
        {
            try
            {
                ...
                for (int i = 0; i < arrayInitCount; i++)
                {
                    ...
                    if (offset + count > data.Length) break;
            ...
            {
                AnsiConsole.MarkupLine($"[red]Error parsing static field arrays: {Markup.Escape(ex.Message)}[/]");
            }
        }
