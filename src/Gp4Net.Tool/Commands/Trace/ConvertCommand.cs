...
namespace Gp4Net.Tool.Commands.Trace
{
    ...
    public class ConvertCommand : AsyncCommand<ConvertCommand.Settings>
    {
        ...
        private void DisplaySummary(TraceData traceData)
        {
            var table = new Table();
    ...
    public class TraceConverter
    {
        ...
        public async Task<TraceData> ConvertAsync(string inputFile, string format, bool verbose = false)
        {
            ...
            var usageExamples = _usageGenerator.GenerateExamples(operations);
        ...
        {
            var description = _apduAnalyzer.GetCommandDescription(command);
            var secureMessaging = _apduAnalyzer.IsSecureMessaging(command);
            var scpData = _apduAnalyzer.ExtractScpData(command, response, description);
        ...
        private void LinkOperationsToSessions(Dictionary<string, Operation> operations, List<SessionMetadata> sessions)
        {
    ...
    public class ApduAnalyzer
    {
        ...
        public string GetCommandDescription(string commandHex)
        {
        ...
        public bool IsSecureMessaging(string commandHex)
        {
        ...
        public ScpData? ExtractScpData(string commandHex, string responseHex, string description)
        {
    ...
    public class OperationDetector
    {
        ...
        {
            {
                "info", new OperationPattern
                {
                    Indicators = new[] { "SELECT", "GET CPLC", "GET CARD DATA", "GET CARD CAPABILITIES", "GET IIN", "GET CIN", "GET KDD", "GET SSC", "GET KEY INFORMATION" },
            ...
            },
            {
                ...
                {
                    Indicators = new[] { "INSTALL [for load]", "LOAD", "INSTALL [for install]" },
        ...
        public Dictionary<string, Operation> AnalyzeTrace(List<Exchange> exchanges)
        {
            ...
            for (int i = 0; i < exchanges.Count; i++)
            {
                ...
                if (detectedOp != currentOperation)
                {
                    // Close previous operation
                    if (currentOperation != null && currentOpName != null && operations.ContainsKey(currentOpName))
                    {
                        operations[currentOpName].EndExchange = i;
                    }

                    // Start new operation
                    currentOperation = detectedOp;
                    currentOpName = GetUniqueOperationName(detectedOp);
                    operationStart = i + 1;

                    operations[currentOpName] = new Operation
                    {
                        Description = GetOperationDescription(detectedOp),
                        SessionId = "session_1", // Will be updated by session analyzer
                        StartExchange = operationStart,
                        EndExchange = i + 1,
                        Commands = new List<string>(),
                        ExpectedCli = OperationPatterns.GetValueOrDefault(detectedOp)?.CliTemplate ?? "gp4net unknown"
                    };
                }

                // Add command to current operation
                if (currentOpName != null && operations.ContainsKey(currentOpName))
                {
                    var operation = operations[currentOpName];
                    if (!operation.Commands.Contains(exchange.Description))
                    {
                        operation.Commands.Add(exchange.Description);
                    }

                    // Update exchange with operation info
                    exchange.Operation = currentOpName;
                    exchange.StepInOperation = exchanges.Take(i + 1).Count(e => e.Operation == currentOpName);
                }
            }
            ...
            if (currentOpName != null && operations.ContainsKey(currentOpName))
            {
                operations[currentOpName].EndExchange = exchanges.Count;
            }

            return operations;
        }

        private string DetectOperationType(Exchange exchange)
        {
    ...
    public class SessionAnalyzer
    {
        ...
        private void UpdateSessionData(SessionMetadata session, Exchange exchange)
        {
    ...
    public class MetadataExtractor
    {
        ...
        private string FindIsdAid(List<Exchange> exchanges)
        {
            foreach (var exchange in exchanges)
            {
        ...
        private CplcData? FindCplcData(List<Exchange> exchanges)
        {
            foreach (var exchange in exchanges)
            {
        ...
        private string DetectCardType(CplcData? cplcData)
        {
    ...
    {
        public List<UsageExample> GenerateExamples(Dictionary<string, Operation> operations)
        {
