...
/// </summary>
public class CardSessionCommands : ICardSessionCommands
{
    private CommandEnvironment _environment;
    private readonly CommandProcessor _processor;
    private readonly ILogger<CardSessionCommands> _logger;
    private bool _disposed;

    /// <summary>
    /// Initializes a new instance of CardSessionCommands.
    /// </summary>
    public CardSessionCommands(
    ...
    /// <inheritdoc/>
    public ImmutablePipelineContext Context
    {
    ...
    )
    {
        ...
        var result = await CardOperations.Run(
            CardOperations.Execute(command, options),
            CardSession.Create(_environment, _processor),
            cancellationToken
        );

        return result.Map(step =>
        {
            _environment = step.Session.Environment;
            return step.Value;
        });
    }
    ...
    )
    {
        ...
        return channelResult
            ...
            .Map(newEnvironment =>
                (ICardSessionCommands)new CardSessionCommands(newEnvironment, _processor, _logger)
            );
    }
    ...
    public Result<ICardSessionCommands, SmartCardError> WithContextValue<T>(string key, T value)
    {
        ...
        if (key == "SecureChannelSession" && value is SecureChannelState secureChannel)
        {
            ...
            return Result.Success<ICardSessionCommands, SmartCardError>(
                new CardSessionCommands(newEnvironment, _processor, _logger)
            );
        }
    ...
    {
        var result = await CardCapabilities.DetectAsync(
            SendCommandAsync,
            _logger,
            cancellationToken
        );
