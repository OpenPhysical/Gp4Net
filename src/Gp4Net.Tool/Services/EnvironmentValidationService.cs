...
namespace Gp4Net.Tool.Services
{
    ...
    public class EnvironmentValidationService : IEnvironmentValidationService
    {
        ...
        private async Task<Result<byte[], SmartCardError>> GetCplcDataAsync(
            ICardChannel channel,
            IApduTransport transport,
        ...
        private CardEnvironment AnalyzeCplcData(byte[] cplcData)
        {
            try
            {
                // Convert CPLC data to string for analysis
                var cplcString = Convert.ToHexString(cplcData);
                var cplcText = System.Text.Encoding.ASCII.GetString(cplcData.Where(b => b >= 32 && b <= 126).ToArray());
        ...
        private async Task<CardEnvironment> AnalyzeCardBehaviorAsync(
            ICardChannel channel,
            IApduTransport transport,
            CancellationToken cancellationToken
        )
