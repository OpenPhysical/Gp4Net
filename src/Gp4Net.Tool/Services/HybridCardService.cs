    ...
    public class HybridCardService : ICardService
    {
        ...
        public IReadOnlyList<string> GetReaders()
        {
            ...
            try
            {
                ...
                var virtualReaders = _virtualCardService.ListReaders().Match(
                    onSuccess: readers => readers,
                    onFailure: error => 
                    {
                        _logger.LogWarning("Failed to get virtual readers: {Error}", error.Message);
        ...
        public bool Connect(string readerName)
        {
            ...
            if (IsVirtualReader(readerName))
            {
                ...
                return _virtualCardService.ConnectToCard(readerName).Match(
                    onSuccess: _ => true,
                    onFailure: error => 
                    {
                        _logger.LogError("Virtual card connection failed: {Error}", error.Message);
                        return false;
                    });
            }
            else
