...
/// </summary>
[PublicAPI]
public sealed class CardStateTransitions
{
    private readonly Maybe<ILogger> _logger;

    /// <summary>
    /// Initializes a new instance of the CardStateTransitions.
    /// </summary>
    /// <param name="logger">Optional logger for debugging state transitions.</param>
    public CardStateTransitions(Maybe<ILogger> logger = default)
    {
        _logger = logger;
    }
    ...
    {
        _logger.Match(
            logger => logger.LogDebug("[CardStateTransitions] {Message}", message),
