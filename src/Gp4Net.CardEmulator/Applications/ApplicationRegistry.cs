...
public sealed record ApplicationRegistry
{
    ...
    {
        return IssuerSecurityDomain.Create(isdAid, scpVersion, scpImplementation)
            .Map(isd =>
            {
    ...
    /// </summary>
    public Result<(ApplicationRegistry UpdatedRegistry, ApduResponse Response), SmartCardError> RouteCommand(
        byte[] command,
        CardState cardState,
    ...
    public Result<ApplicationRegistry, SmartCardError> RemoveApplication(ImmutableArray<byte> aid)
    {
        ...
        var updatedSelection = SelectedApplicationAid.Match(
            selectedAid => selectedAid.SequenceEqual(aid) 
                ? GetIsdAid() 
    ...
    /// </summary>
    public Result<ApplicationRegistry, SmartCardError> UpdateApplication(IApplication updatedApplication)
    {
        if (!Applications.ContainsKey(updatedApplication.Aid))
        {
            return Result.Failure<ApplicationRegistry, SmartCardError>(
                ErrorFactory.ApplicationNotFound(Convert.ToHexString(updatedApplication.Aid.ToArray())));
        }
    ...
    public ImmutableList<IApplication> GetApplicationsByLifecycleState(LifecycleState state)
    {
        return Applications.Values
            .Where(app => app.LifecycleState == state)
            .ToImmutableList();
    }
    ...
    private Result<(ApplicationRegistry, ApduResponse), SmartCardError> SelectIsd(byte fileControlInfo)
    {
        var isdAid = GetIsdAid();
        
        return isdAid.Match(
            aid => Applications.TryGetValue(aid, out var isd)
                ? SelectApplicationInternal(aid, isd, fileControlInfo)
                : Result.Success<(ApplicationRegistry, ApduResponse), SmartCardError>(
                    (this, ApduResponse.Error(Gp4Net.Constants.Constants.StatusWords.Legacy.FileNotFound))),
            () => Result.Success<(ApplicationRegistry, ApduResponse), SmartCardError>(
                (this, ApduResponse.Error(Gp4Net.Constants.Constants.StatusWords.Legacy.FileNotFound))));
    }
        ...
        byte fileControlInfo)
    {
        ...
        var matchingApps = Applications.Values
            .Where(app => app.Aid.Take(targetAid.Length).SequenceEqual(targetAid))
            .ToImmutableList();
        ...
        return matchingApps.Count switch
        {
            ...
            _ => Result.Success<(ApplicationRegistry, ApduResponse), SmartCardError>(
                (this, ApduResponse.Error(Gp4Net.Constants.Constants.StatusWords.Legacy.FileNotFound))) // Multiple matches - ambiguous
        };
    }
        ...
        byte fileControlInfo)
    {
        ...
        return ValidateApplicationSelectable(application)
            .Map(app => 
            {
                var updatedRegistry = this with { SelectedApplicationAid = Maybe<ImmutableArray<byte>>.From(aid) };
                var response = BuildSelectResponse(app, fileControlInfo);
                return (updatedRegistry, response);
            });
    }
    
    private Result<IApplication, SmartCardError> ValidateApplicationSelectable(IApplication application)
    {
        return application.LifecycleState switch
        {
            LifecycleState.Selectable => Result.Success<IApplication, SmartCardError>(application),
            LifecycleState.Personalized => Result.Success<IApplication, SmartCardError>(application),
    ...
    private Result<IApplication, SmartCardError> ValidateApplicationState(IApplication app, byte[] command)
    {
        byte instruction = command[1];
        ...
        CardState cardState)
    {
        ...
        return app.GetRequiredPrivileges(instruction).Match(
            requiredPrivileges =>
            {
                // Check if application has required privileges
                bool hasPrivileges = (app.Privileges & requiredPrivileges) == requiredPrivileges;
            ...
            () => Result.Success<IApplication, SmartCardError>(app)); // No special privileges required
    }
    ...
    private static bool ValidateSecurityLevel(byte instruction, CardState cardState)
    {
        ...
        {
            GpIns.GetStatus => cardState.SecurityLevel >= 0x01,
            ...
            GpIns.PutKey => cardState.SecurityLevel >= 0x01,
            GpIns.StoreData => cardState.SecurityLevel >= 0x01,
            GpIns.SetStatus => cardState.SecurityLevel >= 0x01,
