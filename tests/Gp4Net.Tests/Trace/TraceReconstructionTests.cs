...
public class TraceReconstructionTests
{
    ...
    [TestCase("SCP03", "install_uninstall.json")]
    public void Should_Verify_SCP03_Encryption_And_MAC_With_Actual_Trace_Values(
        string protocol,
        string filename
    )
    {
        // Load the actual SCP03 trace data from JSON
        var traceFile = GetTraceFilePath(protocol, filename);
        ...
        // Get INITIALIZE UPDATE response to extract card challenge
        var exchanges = root.GetProperty("exchanges").EnumerateArray().ToList();
        var initUpdateExchange = exchanges.First(e =>
            e.GetProperty("description").GetString() == "INITIALIZE UPDATE"
        );
        ...
        // Parse card challenge from INITIALIZE UPDATE response
        var cardChallenge = initUpdateResponse.Skip(13).Take(8).ToArray();

        // Find encrypted SCP03 commands
        var encryptedExchanges = exchanges
            .Where(e =>
                e.TryGetProperty("secure_messaging", out var sm)
                && sm.GetBoolean()
        ...
        if (!encryptedExchanges.Any())
        {
            // If no encrypted data, at least verify MAC calculation
            var macExchanges = exchanges
                .Where(e =>
                    e.GetProperty("secure_messaging").GetBoolean()
                    && e.GetProperty("description").GetString() != "EXTERNAL AUTHENTICATE"
                )
            ...
            if (!macExchanges.Any())
            {
                Assert.Ignore($"No secure commands found in {filename}");
                return;
            }

            var macExchange = macExchanges.First();
            var command = Convert.FromHexString(macExchange.GetProperty("command").GetString()!);
            // Parse with WSCT CommandAPDU and extract MAC
            var cmd = new CommandAPDU(command);
            var macExtractResult = cmd.GetMacInput();
            Assert.That(
                macExtractResult.IsSuccess,
                Is.True,
                "Failed to extract MAC from secure SCP03 APDU"
            );
            var actualMac = macExtractResult.Value.ExtractedMac;

            TestContext.Out.WriteLine($"Trace file: {filename}");
            TestContext.Out.WriteLine(
                $"Found secure command with MAC: {Convert.ToHexString(actualMac)}"
            );
            TestContext.Out.WriteLine("Full verification requires master keys from trace");

            // Without the actual keys used in the trace, we can't verify the exact MAC
            // but we've proven the structure is correct
            Assert.Pass(
                $"SCP03 MAC structure verified for {filename}, exact validation requires trace keys"
            );
            return;
        }
        ...
        // Parse encrypted SCP03 command for structural validation
        var scp03EncryptedCmd = new CommandAPDU(encryptedCommand);
        Assert.That(
            scp03EncryptedCmd.Udc.Length,
            Is.GreaterThanOrEqualTo(encryptedData.Length),
            "SCP03 encrypted APDU data must contain encrypted data (and possibly MAC)"
        );

        TestContext.Out.WriteLine($"Trace file: {filename}");
        TestContext.Out.WriteLine($"Host Challenge: {Convert.ToHexString(hostChallenge)}");
        TestContext.Out.WriteLine($"Card Challenge: {Convert.ToHexString(cardChallenge)}");
        TestContext.Out.WriteLine($"Plaintext Data: {Convert.ToHexString(plaintextData)}");
        TestContext.Out.WriteLine($"Encrypted Data: {Convert.ToHexString(encryptedData)}");

        // The test verifies the structure is correct
        Assert.That(
            encryptedData.Length % 16,
            Is.EqualTo(0),
            "Encrypted data must be AES block aligned"
        );
        Assert.Pass(
            $"SCP03 encryption structure verified for {filename}, exact validation requires trace keys"
        );
    }
}
