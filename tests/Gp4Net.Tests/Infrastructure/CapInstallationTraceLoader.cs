...
public static class CapInstallationTraceLoader
{
    ...
    public static Result<CapInstallationTrace, SmartCardError> LoadInstallationTrace(string tracePath)
    {
        return Result.Try(
                () => File.ReadAllText(tracePath),
                ex => SmartCardError.UnexpectedError($"Failed to read trace file: {ex.Message}")
            )
            .Bind(traceContent => Result.Try(
                () => JsonSerializer.Deserialize<JsonDocument>(traceContent),
                ex => SmartCardError.UnexpectedError($"Failed to parse trace JSON: {ex.Message}")
            ))
    ...
    public static Result<CommandSequence, SmartCardError> ExtractCommandSequence(CapInstallationTrace trace)
    {
        return Result.Try(
    ...
    private static Result<byte[], SmartCardError> ParseHexString(string hex)
    {
        ...
        return cleanHex.Length % 2 != 0
            ...
            : Result.Success<byte[], SmartCardError>(
                [.. Enumerable.Range(0, cleanHex.Length / 2).Select(i => Convert.ToByte(cleanHex.Substring(i * 2, 2), 16))]
            );
    }
}
