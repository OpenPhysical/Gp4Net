...
namespace Gp4Net.Tests.CardEmulator.Core;
...
public class CardUuidTests
{
    ...
    {
        // Act
        Result<CardUuid, SmartCardError> result = CardUuid.FromBytes((byte[])null);
    ...
    public void ToByteArray_ReturnsCorrectBytes()
    {
        ...
        uuidResult.Match(
            ...
            error => Assert.Fail($"Failed to create CardUuid: {error.Message}")
        );
    }
    ...
    public void IsEmpty_WithNonEmptyUuid_ReturnsFalse()
    {
        ...
        nonEmptyUuidResult.Match(
            ...
            error => Assert.Fail($"Failed to generate CardUuid: {error.Message}")
        );
    }
    ...
    public void Equality_WithSameGuid_ReturnsTrue()
    {
        ...
        uuid1Result.Match(
            uuid1 => uuid2Result.Match(
                ...
                error => Assert.Fail($"Failed to create second CardUuid: {error.Message}")
            ),
            error => Assert.Fail($"Failed to create first CardUuid: {error.Message}")
        );
    }
    ...
    public void Equality_WithDifferentGuids_ReturnsFalse()
    {
        ...
        uuid1Result.Match(
            uuid1 => uuid2Result.Match(
                ...
                error => Assert.Fail($"Failed to generate second CardUuid: {error.Message}")
            ),
            error => Assert.Fail($"Failed to generate first CardUuid: {error.Message}")
        );
    }
    ...
    public void RoundTrip_ByteArrayConversion_PreservesUuid()
    {
        ...
        originalUuidResult.Match(
            originalUuid => {
                ...
                Result<CardUuid, SmartCardError> reconstructedUuidResult = CardUuid.FromBytes(bytes);

                reconstructedUuidResult.Match(
                    reconstructedUuid => reconstructedUuid.Should().Be(originalUuid),
                    error => Assert.Fail($"Failed to reconstruct CardUuid: {error.Message}")
                );
            },
            error => Assert.Fail($"Failed to generate original CardUuid: {error.Message}")
        );
    }
}
