...
public class Scp02ProtocolModeTests
{
        ...
        string description)
    {
        ...
        // Extract test data
        byte[] staticKeys = Convert.FromHexString(testData.RootElement
            .GetProperty("metadata")
            .GetProperty("hints")
            .GetProperty("static_keys")
            .GetString()!);

        JsonElement session = testData.RootElement
        ...
        int keyVersion = session.GetProperty("key_version").GetInt32();
        string implementationString = session.GetProperty("implementation").GetString()!;

        // Parse implementation parameter (e.g., "i=15" -> 0x15)
        string expectedImplementation = implementationString;
        byte implementationValue = byte.Parse(implementationString[2..], System.Globalization.NumberStyles.HexNumber);
        ScpImplementation scpImplementation = (ScpImplementation)implementationValue;

        // Extract expected session keys
        JsonElement expectedKeys = testData.RootElement
            .GetProperty("metadata")
            .GetProperty("hints")
        ...
        byte[] expectedSRMac = Convert.FromHexString(expectedKeys.GetProperty("s_rmac").GetString()!);
        ...
        Result<Scp02KeySet, SmartCardError> keySetResult = Scp02KeySet.Create(staticKeys, staticKeys, staticKeys, (byte)keyVersion);
        ...
        Result<SessionKeys, SmartCardError> sessionKeysResult = keyDerivation.DeriveSessionKeys(
            keySetResult.Value,
            hostChallenge,
            cardChallenge,
    ...
    public void Scp02_Mode_Should_Calculate_Correct_Cryptograms(string traceFile)
    {
        ...
        byte[] cardChallenge = Convert.FromHexString(session.GetProperty("card_challenge").GetString()!);
        byte[] sequenceCounter = Convert.FromHexString(session.GetProperty("sequence_counter").GetString()!);
        byte[] expectedCardCryptogram = Convert.FromHexString(session.GetProperty("card_cryptogram").GetString()!);
        byte[] expectedHostCryptogram = Convert.FromHexString(session.GetProperty("host_cryptogram").GetString()!);
        ...
        string implementationString = session.GetProperty("implementation").GetString()!;
        byte implementationValue = byte.Parse(implementationString[2..], System.Globalization.NumberStyles.HexNumber);
        ScpImplementation implementation = (ScpImplementation)implementationValue;
        ...
        string description)
    {
        ...
        JsonElement.ArrayEnumerator exchanges = testData.RootElement.GetProperty("exchanges").EnumerateArray();
        ...
        foreach (JsonElement exchange in exchanges)
        {
            ...
            string exchangeDescription = exchange.GetProperty("description").GetString()!;
            ...
            {
                byte cla = Convert.ToByte(command.Substring(0, 2), 16);
                if ((cla & 0x04) != 0)
                {
