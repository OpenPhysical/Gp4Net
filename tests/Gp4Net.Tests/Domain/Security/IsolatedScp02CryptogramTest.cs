...
public class IsolatedScp02CryptogramTest
{
    ...
    public void SCP02_Cryptogram_Should_Match_Real_Card_Response()
    {
        ...
        byte[] actualCardCryptogram = Convert.FromHexString(session.GetProperty("card_cryptogram").GetString()!);
        ...
        Result<Scp02KeySet, SmartCardError> keySetResult = Scp02KeySet.Create(staticKeys, staticKeys, staticKeys, keyVersion);
    ...
    public void SCP02_Cryptogram_Step_By_Step_Debug()
    {
        ...
        byte[] staticKey = Convert.FromHexString(testData.RootElement.GetProperty("metadata").GetProperty("hints").GetProperty("static_keys").GetString()!);
        JsonElement session = testData.RootElement.GetProperty("sessions").GetProperty("session_1");
        byte[] hostChallenge = Convert.FromHexString(session.GetProperty("host_challenge").GetString()!);
        byte[] sequenceCounter = Convert.FromHexString(session.GetProperty("sequence_counter").GetString()!);
        ...
        byte[] sEncDerivationData = new byte[16];
        ...
        Result<byte[], SmartCardError> sEncResult = Scp02Protocol.DeriveScp02SessionKey(
            staticKey,
        ...
        JsonElement expectedKeys = testData.RootElement.GetProperty("metadata").GetProperty("hints").GetProperty("expected_session_keys");
        byte[] knownSEnc = Convert.FromHexString(expectedKeys.GetProperty("s_enc").GetString()!);
        byte[] cardChallenge = Convert.FromHexString(session.GetProperty("card_challenge").GetString()!);
        ...
        Result<byte[], SmartCardError> cryptogramResult = MacCalculations.CalculateScp02Cryptogram(knownSEnc, cryptogramData);
        _ = cryptogramResult.IsSuccess.Should().BeTrue();
