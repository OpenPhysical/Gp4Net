...
namespace Gp4Net.Tests.Domain.Commands
{
    ...
    public class DeleteCommandTests
    {
        ...
        public void CreateForApplication_WithValidAid_ReturnsSuccessResult()
        {
            ...
            command.Aids[0].Should().Equal(aid);
            command.DeletionToken.Should().BeNull();
        }
        ...
        public void CreateForApplication_WithDeletionToken_IncludesToken()
        {
            ...
            result.IsSuccess.Should().BeTrue();
            var command = result.Value;
            command.DeletionToken.Should().Equal(deletionToken);
        }
        ...
        {
            // Act
            var result = DeleteCommand.CreateForApplication(Array.Empty<byte>(), deleteRelated: false);
        ...
        public void CreateForPackage_WithValidAid_ReturnsSuccessResult()
        {
            ...
            result.IsSuccess.Should().BeTrue();
            var command = result.Value;
            command.Aids[0].Should().Equal(packageAid);
        }
        ...
        public void CreateForApplications_WithMultipleAids_ReturnsSuccessResult()
        {
            ...
            {
                command.Aids[i].Should().Equal(aids[i]);
            }
        }
        ...
        public void CreateForKey_WithValidParameters_ReturnsSuccessResult()
        {
            ...
            command.Target.Should().Be(DeleteCommand.DeleteTarget.ByAid);
            command.Aids.Should().HaveCount(1);
            command.Aids[0].Should().Equal(new byte[] { keyIdentifier, keyVersion });
        }
        ...
        public void ToApdu_ForSingleAid_GeneratesCorrectStructure()
        {
            ...
            apdu.Skip(7).Take(8).Should().Equal(aid);
        ...
        public void ToApdu_WithDeletionToken_AppendsTokenDirectly()
        {
            ...
            apdu.Skip(7).Take(aid.Length).Should().Equal(aid);
            
            // Verify deletion token is appended directly (no TLV wrapper)
            var tokenOffset = 7 + aid.Length;
            apdu.Skip(tokenOffset).Take(deletionToken.Length).Should().Equal(deletionToken);
        }
        ...
        public void ToApdu_ForMultipleAids_ConcatenatesAllAids()
        {
            ...
            {
                apdu.Skip(offset).Take(aid.Length).Should().Equal(aid);
                offset += aid.Length;
            }
        }
        ...
        public void Data_Property_ReturnsCorrectTlvStructure()
        {
            ...
            data.Skip(2).Take(aid.Length).Should().Equal(aid);
        }
        ...
        public void DeleteResponse_Parse_WithDeletionReceipt()
        {
            ...
            response.DeletionReceipts[0].Aid.Should().Equal(deletedAid);
        ...
        public void DeleteResponse_GetResultDescription_MapsStatusWords()
        {
            ...
            {
                // Act
                var response = new DeleteResponse(Array.Empty<byte>(), statusWord);
        ...
        public void DeleteCommand_IsImmutable()
        {
            ...
            // Assert
            command.Aids.Should().HaveCount(1);
            command.Aids[0].Should().Equal(originalAids[0]);
        }
