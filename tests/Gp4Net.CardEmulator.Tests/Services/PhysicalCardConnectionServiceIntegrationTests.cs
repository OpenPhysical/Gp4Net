...
{
    private static readonly Lazy<string> ProfilePath = new(() =>
        Path.Combine(
    ...
    public async Task Should_Create_Service_For_Virtual_Reader()
    {
        ...
        var selectCommand = Gp4Net.Services.GlobalPlatform.Commands
    ...
    {
        string readerSpec = $"virtual:{Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("N"))}.json";
